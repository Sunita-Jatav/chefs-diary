...
// Shows the complete recipe with emotional story, ingredients,
// steps, and the interactive cooking assistant (Groq streaming).
...
const CookingAssistant = ({ recipe }) => {
  ...
  const sendMessage = async () => {
    ...
    // When stream is done, add assistant message to history
    setMessages(prev => [
      ...prev,
...
export const RecipePage = ({ toast }) => {
  ...
  const handleSubstitution = async () => {
    ...
    try {
      const res = await recipeAPI.getBySlug; // use recipe already loaded
  ...
  if (error || !recipe) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', textAlign: 'center' }}>
        <div>
          <p style={{ fontFamily: 'var(--font-display)', fontSize: '4rem', marginBottom: '1rem' }}>🍽️</p>
          <p style={{ color: 'var(--color-ink-muted)' }}>Recipe not found.</p>
          <Link to="/" style={{ color: 'var(--color-terracotta)', marginTop: '1rem', display: 'block' }}>
            ← Back to Discover
          </Link>
        </div>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--color-parchment)' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '2rem 1rem 6rem' }}>
        ...
        {substitution && (
          <div style={{
            background: 'rgba(122,158,114,0.08)',
            ...
            {substitution.substitutions?.map((s, i) => (
              <div key={i} style={{
                ...
                <span style={{ color: 'var(--color-ink-muted)', minWidth: '8rem', fontWeight: 500 }}>
                  {s.original}
                </span>
                ...
                <div>
                  ...
                  {s.note && (
                    <span style={{ color: 'var(--color-ink-muted)', fontSize: '0.78rem' }}> · {s.note}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        <div style={{
          ...
          {recipe.ingredients?.map((ing, i) => (
            <div
              ...
              style={{
                ...
                borderBottom: i < recipe.ingredients.length - 1
                  ? '1px solid rgba(44,31,14,0.06)' : 'none',
            ...
            >
              ...
              <span style={{ color: 'var(--color-ink)', fontSize: '0.9rem', flex: 1 }}>
                {ing.name}
              </span>
              {ing.notes && (
                <span style={{ color: 'var(--color-ink-muted)', fontSize: '0.78rem', fontStyle: 'italic' }}>
                  {ing.notes}
                </span>
              )}
              {ing.isOptional && (
          ...
          {recipe.steps?.map((step, i) => (
            <div
              ...
              style={{
                ...
                cursor: 'pointer',
                transition: 'all 0.2s',
                boxShadow: activeStep === i ? 'var(--shadow-warm)' : 'none',
              }}
            >
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  {/* Duration + tips — shown when active */}
                  {activeStep === i && (
